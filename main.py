...
class GuassianDiffusion:
    ...
    def __init__(self, timesteps=1000, device="cuda:0"):
        ...
        self.device = device
        self.alpha_bar_scheduler = (lambda t: math.cos((t / self.timesteps + 0.008) / 1.008 * math.pi / 2) ** 2)
        self.scalars = self.get_all_scalars(self.alpha_bar_scheduler, self.timesteps, self.device)
        ...
        self.get_pred_mean_from_x0_xt = (
            ...
            + unsqueeze3x((scalars.alpha[t] - scalars.alpha_bar[t]) / ((1 - scalars.alpha_bar[t]) * scalars.alpha[t].sqrt())) * xt)
    ...
    def get_all_scalars(self, alpha_bar_scheduler, timesteps, device, betas=None):
        ...
        if betas is None:
            ...
            all_scalars["beta"] = torch.from_numpy(np.array([min(1 - alpha_bar_scheduler(t + 1) / alpha_bar_scheduler(t), 0.999) for t in range(timesteps)])).to(device)
        ...
        all_scalars["beta_tilde"] = all_scalars["beta"][1:] * (1 - all_scalars["alpha_bar"][:-1]) / (1 - all_scalars["alpha_bar"][1:])
        all_scalars["beta_tilde"] = torch.cat([all_scalars["beta_tilde"][0:1], all_scalars["beta_tilde"]])
    ...
    def sample_from_forward_process(self, x0, t):
        ...
        xt = unsqueeze3x(self.scalars.alpha_bar[t].sqrt()) * x0 + unsqueeze3x((1 - self.scalars.alpha_bar[t]).sqrt()) * eps
    ...
    for step, (images, labels) in enumerate(dataloader):
        ...
        if args.local_rank == 0:
            ...
            for (k, v) in args.ema_dict.items():
                ...
                args.ema_dict[k] = (args.ema_w * args.ema_dict[k] + (1 - args.ema_w) * new_dict[k])

            logger.log(loss.item(), display=not step % 100)


def sample_N_images(N,
        model,
        diffusion,
        xT=None,
        sampling_steps=250,
    ...
    with tqdm(total=math.ceil(N / (args.batch_size * num_processes))) as pbar:
        ...
        while num_samples < N:
            ...
            if xT is None:
                ...
                xT = torch.randn(batch_size, num_channels, image_size, image_size).float().to(args.device)
            ...
            gen_images = diffusion.sample_from_reverse_process(model, xT, sampling_steps, {"y": y}, args.ddim)
...
def main():
    ...
    parser.add_argument("--num-sampled-images", type=int, default=50000,
    ...
    model = unets.__dict__[args.arch](
        ...
        num_classes=metadata.num_classes if args.class_cond else None).to(args.device)
    ...
    if (ngpus := torch.cuda.device_count()) > 1:
    ...
    for epoch in range(args.epochs):
        ...
        if not epoch % 1:
            ...
            sampled_images, _ = sample_N_images(64,
                                                model,
                                                diffusion,
                                                None,
                                                args.sampling_steps,
